...
app.get('/users/:id/logs', async (req, res) => {
  try {
    const { id } = req.params;
    const db = client.db(dbName);
    const logs = await db.collection('userLogs').find({ user_id: id }).toArray();
    res.json({
      code: 200,
      message: 'success',
      data: logs,
...
app.put('/cart/:id', async (req, res) => {
  try {
    ...
    const result = await db
      .collection('cart')
      .findOneAndUpdate({ id }, { $set: updatedItem }, { returnDocument: 'after' });
...
app.get('/article_comments', async (req, res) => {
  try {
    ...
    const db = client.db(dbName);
    const comments = await db.collection('article_comments').find({ article_id }).toArray();
    res.json(comments);
  } catch (err) {
    res.status(500).json({
      code: 500,
...
app.get('/product_comments', async (req, res) => {
  try {
    ...
    const db = client.db(dbName);
    const comments = await db.collection('product_comments').find({ product_id }).toArray();
    res.json(comments);
  } catch (err) {
    res.status(500).json({
      code: 500,
...
app.post('/feedback', async (req, res) => {
  try {
    const newItem = req.body;
    // 生成唯一id（时间戳+随机数）
    newItem.id = `FDBK${Date.now()}${Math.floor(Math.random() * 10000)}`;
...
// 用户更新相关接口
app.put('/users/:user_id', upload.single('user_avatar'), async (req, res) => {
  try {
    const { user_id } = req.params;
    const updateData = req.body;
    const db = client.db(dbName);
    ...
    const result = await db
      .collection('users')
      .findOneAndUpdate({ id: user_id }, { $set: setData }, { returnDocument: 'after' });
  ...
  try {
    ...
    if (newAddress.is_default) {
      await db
        ...
        .updateMany({ user_id: newAddress.user_id }, { $set: { is_default: false } });
    }
...
app.put('/addresses/:id', async (req, res) => {
  try {
    ...
    if (updateData.is_default) {
      await db
        .collection('addresses')
        .updateMany({ user_id: updateData.user_id }, { $set: { is_default: false } });
    }
...
app.delete('/addresses/:id', async (req, res) => {
  try {
    ...
    const result = await db.collection('addresses').findOneAndDelete({ _id: new ObjectId(id) });
...
app.post('/normal_orders', async (req, res) => {
  try {
    ...
    // 生成唯一订单ID（可用时间戳+随机数）
    newOrder.id = `ORD${Date.now()}${Math.floor(Math.random() * 10000)}`;
...
app.delete('/normal_orders/:id', async (req, res) => {
  try {
    ...
    const result = await db.collection('normal_orders').findOneAndDelete({ _id: new ObjectId(id) });
...
app.post('/custom', async (req, res) => {
  try {
    ...
    // 生成唯一id（时间戳+随机数）
    newCustom.id = `CSTM${Date.now()}${Math.floor(Math.random() * 10000)}`;
    newCustom.created_at = new Date().toISOString();
    const db = client.db(dbName);
...
// 更改定制信息接口
// filepath: c:\Users\33404\Desktop\收纳\florum\src\mock-data\app.js
// 更改定制信息接口
